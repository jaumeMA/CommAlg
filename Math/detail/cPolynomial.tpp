...
typename cPolynomialValue<T,A>::underlying_type& cPolynomialValue<T,A>::getAt(signed short key, Args&& ... keys)
{
...
template<typename T, template<typename> class A>
template<typename ... Args>
const typename cPolynomialValue<T,A>::underlying_type& cPolynomialValue<T,A>::getAt(signed short key, Args&& ... keys) const
{
    if(this->m_pNodeLocation->m_value.first == 0 && mpl::are_ranks_equal_to<0>(key, keys...))
    {
        return this->m_value;
