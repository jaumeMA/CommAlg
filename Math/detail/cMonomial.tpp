...
{}
template<typename T>
cMonomial<T>::cMonomial(const T& i_coeff, const container::cArray<signed short>& i_poweredIndexes)
: m_coeff(i_coeff)
...
cMonomial<T> cMonomial<T>::operator*(const cMonomial<T>& other) const
{
    ...
    yame::merge(m_poweredIndexes, other.m_poweredIndexes, res.m_poweredIndexes, [](ytl::optional<int> i_indexA, ytl::optional<int> i_indexB){ signed short res = 0; if(i_indexA) res += *i_indexA; if(i_indexB) res += *i_indexB; return res;});
...
ytl::optional<cMonomial<T>> cMonomial<T>::operator/(const cMonomial<T>& other) const
{
    ...
    yame::merge(m_poweredIndexes, other.m_poweredIndexes, res.m_poweredIndexes, ytl::function<signed short(ytl::optional<const signed short&>,ytl::optional<const signed short&>)>([](ytl::optional<const signed short&> i_indexA, ytl::optional<const signed short&> i_indexB){ signed short res = 0; if(i_indexA)res=*i_indexA; if(i_indexB)res-=*i_indexB; return res; }));

    //check that all of our indexes are bigger than other's
    if(yame::find_if(res.m_poweredIndexes, ytl::function<bool(const signed short&)>([](signed short i_index){ return i_index < 0; })) != res.m_poweredIndexes.end())
    {
        return none;
    }
    else
...
template<typename T>
void cMonomial<T>::setPoweredIndex(size_t i_index, unsigned short i_power)
{
...
template<typename T>
void cMonomial<T>::insertPoweredIndex(size_t i_pos, unsigned short i_power)
{
    m_poweredIndexes.insert(m_poweredIndexes.begin() + i_pos, i_power);
}
template<typename T>
unsigned short cMonomial<T>::getPowerOfIndex(size_t i_index) const
{
...
template<typename T>
unsigned short cMonomial<T>::getTotalOrder() const
{
...
bool cMonomial<T>::_lexMoreComparison(const cMonomial<T>& i_lhs, const cMonomial<T>& i_rhs, bool leftToRight)
{
    ...
    {
        const container::cArray<signed short>& lhsIndexes = i_lhs.m_poweredIndexes;
        const container::cArray<signed short>& rhsIndexes = i_rhs.m_poweredIndexes;
