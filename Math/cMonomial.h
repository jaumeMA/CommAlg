...
{
public:
    typedef container::const_array_view<signed short> power_index_view;

    cMonomial();
    cMonomial(const T& i_coeff, const container::cArray<signed short>& i_poweredIndexes);
    ...
    T getCoeff() const;
    void insertPoweredIndex(size_t i_pos, unsigned short i_power);
    void setPoweredIndex(size_t i_index, unsigned short i_power);
    size_t getNumIndexes() const;
    unsigned short getPowerOfIndex(size_t i_index) const;
    unsigned short getTotalOrder() const;
...
private:
    ...
    T m_coeff = T();
    container::cArray<signed short> m_poweredIndexes;
};
