...
, m_maxLoadFactor(1.f)
{
    ...
    for(signed short i=0;i<m_maxNumBuckets;i++)
    {
...
{
    return reinterpret_cast<iterator_pair*>((char*)memcpy((char*)memcpy(detail::k_pairAllocator.allocate(1, sizeof(iterator_pair)),&i_currNode, sizeof(i_currNode)) + sizeof(i_currNode),&i_currPair, sizeof(i_currPair)) - sizeof(i_currNode));
}
template<typename T, typename R, typename N, typename hash, template<typename> class A>
