...
cBuddyAllocator::cPool::cBuddyFreeList::cBuddyFreeList(unsigned short i_size)
...
void* cBuddyAllocator::cPool::cBuddyFreeList::getBuddyAt(void* i_address) const
{
    ...
    while(currBuddy && (char*)currBuddy < (char*)i_address)
    {
...
cBuddyAllocator::cPool::cPool(void* address, unsigned int totalSize)
: m_pAddress(address)
, m_totalSize(totalSize)
{
    unsigned short powerOfTwoBitPos = math::powerOf2BitPos(totalSize);
...
void* cBuddyAllocator::cPool::allocate(size_t size)
{
    ...
    unsigned short nextPowerOf2BitPos = math::powerOf2BitPos(size);
    ...
    for(size_t currIndex = nextPowerOf2BitPos;currIndex < m_buddyBlocks.getSize();++currIndex)
    {
        ...
        if(buddyBlock.thereAreFreeBuddies())
        {
            ...
            char* absResAddress = reinterpret_cast<char*>(m_pAddress) + nextAvailableBuddy;
...
void cBuddyAllocator::cPool::deallocate(void* address)
{
    ...
    typename container::cTrie<unsigned short,container::cByteSet<>>::iterator_type itAllocation = m_allocations.search(size_t(address));

    size_t localAddress = (char*)address - (char*)m_pAddress;
...
void* cBuddyAllocator::AllocatePool(void* initialAddress, unsigned int length) const
{
    void* res = NULL;
